...
export const generateOtp = (identifier: any, expires_in?: number, length: number = 6): Promise<any> => {
    let otp = genOtp(length); //generate the otp
    let addTime = expires_in ? Number(expires_in) : 30; //😛😛 i am stupid => o .. will make this batter
    let expires_at = new Date().setMinutes(new Date().getMinutes() + addTime); //the otp only valid for 30 minutes if not input
